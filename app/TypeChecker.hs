...
checkT s c (Abs x a b) U = do
  ...
  checkT s c' b U
checkT s c (Abs x a m) (Clos (Abs x' a' t) r) = do
  checkT s c a U
  let r0 = getEnv s c
      va  = eval a r0
      va' = eval a' r
...
convertBeta ns (Clos (Abs x a m) r) (Clos (Abs x' a' m') r') = do
  let va  = eval a r
      va' = eval a' r'
...
convertCheckExpr s ctc cc ac ce =
  ...
  where
    ...
    soundExpr ctx e@(Abs x a b) =
      case runG (checkD s ctx (Dec x a)) (CNil, ctc) of
        Left err   -> Left $ unlines . map errorMsg $ explain err
        Right ctx' -> case soundExpr ctx' b of
                         Left err -> Left err
                         Right _  -> Right e
    soundExpr ctx e@(Let x a b m) =
      case runG (checkD s ctx (Def x a b)) (CNil, ctc) of
        Left err   -> Left $ unlines . map errorMsg $ explain err
